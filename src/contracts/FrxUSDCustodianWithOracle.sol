 ...
 * ====================================================================
 * FrxUSD Custodian contract for the Frax protocol to custody tokens at an oracle price
...
contract FrxUSDCustodianWithOracle is FrxUSDCustodian {
    ...
    /// @notice Constructor
    /// @param _frxUSD The address of the FRAX stablecoin
    ...
    function deposit(uint256 _assetsIn, address _receiver) public override updateOracle returns (uint256 _sharesOut) {
    ...
    function withdraw(
        ...
        address _receiver,
        address _owner
    ) public override updateOracle returns (uint256 _sharesIn) {
    ...
    function redeem(
        ...
        address _receiver,
        address _owner
    ) public override updateOracle returns (uint256 _assetsOut) {
    ...
    function getCustodianOraclePrice() public view returns (uint256 _price) {
        if (lastOracleUpdate == block.number) {
            ...
            (, int256 _answer, , uint256 _chainlinkUpdatedAt, ) = AggregatorV3Interface(custodianOracle)
                .latestRoundData();
    ...
    ) internal view override returns (uint256 _shares) {
        _shares = Math.mulDiv(
            super._convertToShares(_assets, _rounding),
    ...
    function _convertToAssets(
        uint256 _shares,
        Math.Rounding _rounding
    ) internal view override returns (uint256 _assets) {
        _assets = Math.mulDiv(
