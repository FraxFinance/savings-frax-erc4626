...
import { FrxUSDCustodianWithOracle } from "./FrxUSDCustodianWithOracle.sol";

contract FrxUSDCustodianWithOracleFactory is Ownable2Step {
    /* ========== STATE VARIABLES ========== */
    ...
    function deployCustodian(
        ...
        address _implementation = address(new FrxUSDCustodianWithOracle(frxUSD, _custodianTkn));
        _custodianAddr = address(new TransparentUpgradeableProxy(address(_implementation), owner(), ""));
        FrxUSDCustodianWithOracle(_custodianAddr).initialize(
            owner(),
            _custodianOracle,
            _maximumOracleDelay,
            _mintCap,
